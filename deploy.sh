...
EMAIL_SSH_KEY="example@gmail.com"
GIT_REPO_URL="https://github.com/mygituser/example.git"
...
if [ -f "$SSH_KEY_PATH" ]; thì
  echo "SSH key đã tồn tại tại $SSH_KEY_PATH"
else
  # Tạo SSH key
  echo "Tạo SSH key..."
  ssh-keygen -t rsa -b 4096 "$EMAIL_SSH_KEY" -f "$SSH_KEY_PATH" -N ""
fi
...
ssh $SERVER_USER@$SERVER_IP -i $SSH_KEY_PATH << EOF
  ...
  cd $REMOTE_PROJECT_PATH/AIC-2024/Download_video
